...
class TemplateService {
     ...
     */
    applySheetTemplate(spreadsheetId, sheetName, template, listDataMap) {
        ...
        template.fields.forEach(field => {
            ...
            if (field.label)
                range.setValue(field.label);
            if (field.align)
                range.setHorizontalAlignment(field.align);
            if (field.verticalAlign)
                range.setVerticalAlignment(field.verticalAlign);
            if (field.rotation)
                range.setTextRotation(field.rotation);
            if (field.fontWeight)
                range.setFontWeight(field.fontWeight);
        ...
        template.merges.forEach(merge => {
            try {
                ...
                Logger.log(`Advertencia: No se pudo combinar el rango ${merge.range}: ${e.message}`);
            }
        });
     ...
     */
    insertImagesFromTemplate(sheet, template) {
        ...
        template.images.forEach((imageInfo) => {
            try {
                ...
                Logger.log(`Error al insertar la imagen para la key ${imageInfo.logoKey}: ${e.message}`);
            }
        });
    }
}
