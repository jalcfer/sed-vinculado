...
class PaService_ {
     ...
     */
    subirInformeBimensual(paEmail, fileData) {
     ...
     */
    generarInformePreliminar(paEmail, reportProgress) {
        reportProgress('Obteniendo información del PA...');
        const pa = this.acompanamientoRepository.findPaByEmail(paEmail);
        if (!pa)
            throw new Error("No se encontró al PA en la base de datos.");
        ...
        Object.keys(jornadasData).sort((a, b) => parseInt(a) - parseInt(b)).forEach(numJornada => {
            ...
            Array.from(data.objetivos).forEach(item => body.appendListItem(item).setGlyphType(DocumentApp.GlyphType.BULLET));
            body.appendParagraph('Logros').setHeading(DocumentApp.ParagraphHeading.HEADING2);
            Array.from(data.logros).forEach(item => body.appendListItem(item).setGlyphType(DocumentApp.GlyphType.BULLET));
            body.appendParagraph('Dificultades').setHeading(DocumentApp.ParagraphHeading.HEADING2);
            Array.from(data.dificultades).forEach(item => body.appendListItem(item).setGlyphType(DocumentApp.GlyphType.BULLET));
        });
        doc.saveAndClose();
