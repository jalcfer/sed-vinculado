...
function onOpen(e: GoogleAppsScript.Events.DocsOnOpen) {
  try {
    ...
    Logger.log('Error en onOpen (probablemente requiere autorización): ' + (error as Error).message);
    createOnOpenAuthorizeMenu();
  }
}
...
function onOpenAuthorize() {
  try {
    ...
    // onOpen({} as any); 
    buildDynamicMenu({} as any);

  } catch (e) {
    Logger.log('El usuario no concedió la autorización. ' + (e as Error).message);
 ...
 */
function buildDynamicMenu(e: GoogleAppsScript.Events.DocsOnOpen) {
  // Si está autorizado, construir el menú dinámico.
  try {
    getMenuService().createDynamicMenu();
  } catch (error) {
    Logger.log('Error al crear el menú dinámico: ' + (error as Error).message);
