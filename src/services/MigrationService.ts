...
class MigrationService_ {
  ...
  public static getInstance(): MigrationService_ {
   ...
   */
  public migrateEvidencias(dbOrigenId: string, reportProgress: (message: string) => void): string {
    ...
    const evidenciasOrigen = dbOrigen.selectFrom('Evidencia', ['ID_Registro_evidencia', 'ID_visita', 'Nombre_archivo_original', 'Url']).execute() as Evidencia[];
    ...
    evidenciasOrigen.forEach(evidencia => {
      try {
        ...
        if (!fileId) throw new Error(`URL de evidencia no válida en origen: ${evidencia.Url}`);
        ...
        if (!parentFolders.hasNext()) throw new Error(`El archivo de evidencia ${evidenceFile.getName()} no tiene carpeta padre.`);
        ...
        if (!jornadaParentFolders.hasNext()) throw new Error(`La carpeta de evidencias para ${evidenceFile.getName()} no tiene padre.`);
        ...
        if (!visitaOrigen) throw new Error(`No se encontró la visita ${evidencia.ID_visita} en la BD de Origen.`);

        const acompanamientoDestino = acompanamientosDestinoMap.get(visitaOrigen.ID_Acompanamiento);
        if (!acompanamientoDestino) throw new Error(`No se encontró el acompañamiento ${visitaOrigen.ID_Acompanamiento} en la BD de Destino.`);
   ...
   */
  public migrateSemiManual(migrationData: { sourceFolderId: string; destinationFolderId: string }[]): string {
    let successCount = 0;
    let filesFound = 0;
    let errorCount = 0;
    ...
    for (const [index, pair] of migrationData.entries()) {
      const urlsMovidas: string[] = [];
      let sourceFolder: GoogleAppsScript.Drive.Folder;
      ...
      try {
          ...
          try {
            ...
            // if (evidenciaMapByDriveId.has(originalFileId)) {
              ...
              try {
                ...
                errorCount++;
                Logger.log(`Error moviendo archivo ${file.getName()}: ${(moveError as Error).message}`);
              }
            //}
            ...
            errorCount++;
            Logger.log(`Error procesando archivo: ${(fileError as Error).message}`);
            continue;
          }
        }
        ...
        if (urlsMovidas.length > 0) {
          try {
            ...
            let jornadaFile: GoogleAppsScript.Drive.File | null = null;
            ...
            errorCount++;
            Logger.log(`Error actualizando hoja de jornada: ${(sheetError as Error).message}`);
          }
        }
      } catch (e) {
        errorCount++;
        Logger.log(`Error procesando par de carpetas (Origen: ${pair.sourceFolderId}): ${(e as Error).message}`);
      }
    }
...
function getMigrationService(): MigrationService_ {
...
function runEvidenciaMigration_() {
  ...
  getNotificationService().runProcessWithFeedback(
    ...
    'Iniciando proceso de migración. Esto puede tardar varios minutos.',
