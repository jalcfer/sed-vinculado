 ...
 */
function buscarDocentePorIdentificacion(tipoId: string, numeroId: string): DocenteCompletoDTO | null {
  try {
    return getDocenteService().getDocenteCompletoByIdentificacion(tipoId, numeroId);
  } catch (e) {
    Logger.log(`Error en buscarDocentePorIdentificacion: ${(e as Error).message}`);
 ...
 */
function guardarDocente(data: DocenteCompletoDTO): { success: boolean; message: string; docenteId?: number } {
  try {
    ...
    Logger.log(`Error en guardarDocente: ${(e as Error).message}`);
    throw e; // Re-lanzar para que el cliente lo maneje
  }
}
