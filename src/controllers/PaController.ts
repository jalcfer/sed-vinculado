 ...
 */
function crearMultiplesJornadas(acompId: string, numeroDeJornadas: number) {
 ...
 */
function subirInformeBimensual(fileData: { name: string; mimeType: string; data: string }): { success: boolean; message?: string; fileUrl?: string } {
  try {
    const userEmail = PropertiesService.getScriptProperties().getProperty(appConfig.properties.USER_EMAIL_KEY);
    if (!userEmail || userEmail.trim() === '') {
